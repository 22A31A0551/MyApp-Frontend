...
function Navbar({ setShowLogin, isLoggedIn, setIsLoggedIn, userRole, setPage }) {
  ...
  useEffect(() => {
    if (!isLoggedIn) {
      setExpiringLoans([]); // Clear loans if logged out
      return;
    }
  ...
  return (
    <nav style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
        ...
        {/* Notification & Profile (Only shown when logged in) */}
        {isLoggedIn && (
          <>
            {/* Notification */}
            <div
            ...
            >
              ...
              {showNotificationDropdown && (
                <div style={{
                  position: "absolute",
                  top: "100%",
                  right: "-50px", // Center slightly relative to the icon
                  paddingTop: "15px", // Acts as an invisible bridge without creating a gap
                  width: "280px",
                  zIndex: 1001,
                  animation: "fadeIn 0.2s ease-out"
                }}>
                  <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 15px", borderBottom: "1px solid #e2e8f0" }}>
                    ...
                    <div style={{ padding: "10px" }}>
                      {expiringLoans.length > 0 ? (
                        expiringLoans.slice(0, 2).map((loan, idx) => (
                          <div key={loan._id || idx} style={{
                            padding: "10px",
                            borderBottom: idx < 1 && expiringLoans.length > 1 ? "1px solid #e2e8f0" : "none",
                            display: "flex",
                            flexDirection: "column",
                            gap: "4px"
                          }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <span style={{ fontWeight: "700", fontSize: "14px", color: "#000" }}>{loan.name}</span>
                              <span style={{ color: "#ef4444", fontSize: "12px", fontWeight: "700" }}>Expiring Soon</span>
                            </div>
                            <span style={{ fontSize: "13px", color: "#1e293b", fontWeight: "500" }}>Amount: ₹{loan.amount}</span>
                          </div>
                        ))
                      ) : (
                        <div style={{ padding: "15px", textAlign: "center", color: "#000", fontSize: "13px" }}>
                          No expiring loans
                        </div>
                      )}
                    </div>
